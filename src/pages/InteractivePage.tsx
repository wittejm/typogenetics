    ...
    <div>
      <div className="two-panel">
        <div className="strand-panel">
          <p>
            {"Here you can interact directly with the rules of the game " +
              "to build some intuition about its dynamics. "}
          </p>
          <p>
            {"Select strands from the pool below to run any enzyme against any strand " +
              "(including its own source). When selecting the target strand, you must " +
              "select the base that it should bind to if there is more than one valid target. " +
              "You can also create your own strands to add to the available pool."}
          </p>
          {(() => {
            const previewEnzymes =
              newStrand.length >= 2 ? translate(newStrand) : [];
            return (
              <div className="strand-create-section">
                ...
                <div className="strand-create-row">
                  ...
                  {previewEnzymes.length > 0 && (
                    <span className="strand-create-binding">
                      {previewEnzymes.map((enz, i) => (
                        <span key={i} className="bind-pref">
                          binds {enz.bindingPref}
                        </span>
                      ))}
                    </span>
                  )}
                  <button
                    ...
                    onClick={() => {
                      setStrands((prev) => [newStrand, ...prev]);
                      setNewStrand("");
                    }}
                  >
                    Create
          ...
          <ol
            className="strand-list"
            style={
              { "--exit-duration": `${0.5 / speed}s` } as React.CSSProperties
            }
          >
            {strands.map((strand, i) => {
              ...
              const isExiting =
                selected?.stage === "bound" &&
                (selected.si === i || selected.ti === i);

              const isValidTarget =
                selected?.stage === "enzyme" &&
                selectedBindingPref !== null &&
                strand.includes(selectedBindingPref);
              const isInert =
                selected?.stage === "enzyme" && !isSource && !isValidTarget;
              ...
              return (
                ...
                >
                  <StrandText
                    ...
                    onBaseClick={
                      isValidTarget ? (bi) => handleBaseClick(i, bi) : null
                    }
